...
def fetch_papers(
    topic: Dict[str, Any], storage: Storage, since: Optional[str] = None
) -> Tuple[List[Dict[str, Any]], str]:
    """Fetch papers for a topic with date gating."""
    try:
        # arxiv API rate limiting is enabled by default in the Client (1 request every 3 seconds)
        client = arxiv.Client()
        search = arxiv.Search(
            query=topic["query"],
...
def preview_papers(query: str, limit: int = 5) -> List[Dict[str, Any]]:
    """Fetch recent papers for preview (no storage)."""
    try:
        client = arxiv.Client()
        search = arxiv.Search(
            query=query,
