...
from arxiv_scanner.core.ingest import fetch_papers
...
def add_topic(
    ...
    with console.status("[bold green]Generating arXiv query..."):
        try:
            ...
            console.print(f"[green]✓[/green] Topic added with query: [cyan]{query}[/cyan]")
...
def update(
    ...
    for topic in topics:
        ...
        for paper in new_papers:
            ...
            result = process_paper(paper['title'], paper['abstract'], topic['description'], llm_provider)
...
def digest(topic_id: int = typer.Argument(..., help="Topic ID")):
    ...
    for p in papers:
        ...
        if summary:
            ...
            console.print(f"[bold]Key Contribution:[/bold] {summary.get('key_contribution')}")
